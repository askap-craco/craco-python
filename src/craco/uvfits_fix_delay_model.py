...
def fix_delay_model(data_block, new_w_terms, old_w_terms, channel_freqs):
    w_diff = new_w_terms - old_w_terms
    phasors = w_diff[:, None] * channel_freqs[None, :]
    print(f"new_w_terms -{new_w_terms.shape}, old_w_terms - {old_w_terms.shape}, data_block - {data_block.shape}, phasors - {phasors.shape}")
    correction_term = np.exp(-1j * 2* np.pi * phasors)      #I am putting a -1 here because I have to divide the phasor

    return data_block * correction_term[:, None, None, None, :, None, None]

def main(args):
    f_old = uvfits_meta.open(args.uvpath, metadata_file = args.metadata, skip_blocks = args.tstart, mask=args.apply_metadata_masks)
    f = uvfits_meta.open(args.uvpath, metadata_file = args.metadata, skip_blocks = args.tstart, mask=args.apply_metadata_masks, calc11=True)
    ...
    old_blocker = f_old.fast_raw_blocks(nsamp = nsamps_to_read, nt = 1, raw_date = True)
    try:
        for iblk, visout in enumerate(f.fast_raw_blocks(nsamp = nsamps_to_read, nt = 1, raw_date=True)):
            ...
            data_block_old = next(old_blocker)

            new_w_terms = visout['WW'].squeeze()
            old_w_terms = data_block_old['WW'].squeeze()

            data_block = fix_delay_model(data_block, new_w_terms, old_w_terms, f.channel_frequencies)
